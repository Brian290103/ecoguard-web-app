...
const ReportsClientPage = ({
  ...
  const handleStatusUpdate = (
    id: string,
    status: "acknowledged" | "inprogress" | "escalated" | "resolved",
  ) => {
                ...
                <CardHeader>
                  ...
                  <CardAction>
                    {report.status === "submitted" && (
                      <Button
                        onClick={() =>
                          handleStatusUpdate(report.id, "acknowledged")
                        }
                    ...
                    {report.status === "acknowledged" && (
                      <div className="flex gap-2">
                        <Button
                          onClick={() =>
                            handleStatusUpdate(report.id, "inprogress")
                          }
                          disabled={updateReportMutation.isPending}
                        >
                          {updateReportMutation.isPending ? (
                        ...
                        <Button
                          onClick={() =>
                            handleStatusUpdate(report.id, "escalated")
                          }
                          disabled={updateReportMutation.isPending}
                        >
                          {updateReportMutation.isPending ? (
